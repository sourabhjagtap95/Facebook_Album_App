...
<head>
    ...
    <style type="text/css">
        ...
        body{
            background: url(libs/resources/images/facebook.png) no-repeat center center fixed;
...
if(!empty($_SESSION['facebook_access_token']) && isset($_SESSION['facebook_access_token'])) {
    ...
    if(!isset($_GET['album_name'])){
        echo "<div class='text-center jumbotron'><h2>Please Select An Album</h2></div>";
    }
}
else
{
...
//                                            file_put_contents($photos_directory . "/" . uniqid() . ".jpg", fopen($albums_photo_all['source'], 'r'));
    ...
    if(!isset($_GET['google_session'])){
        echo "<script>window.location.href='libs/login_to_google.php';</script>";
    }
}
